# SmartNews広告入稿CSV自動生成ツール（Google Apps Script (GAS) + Google スプレッドシート）
...
スプレッドシート関数が自動で広告パターンを展開し整形、Google Apps Script (GAS)が**媒体指定のCSVファイル形式に出力**します。
...
| `06_SN_INPUT` | 04_SN_BANNERと05_SN_TEXTをもとに、入稿したい広告パターンを作成。スクリプトを割り当てたボタンをクリックすることで、Google Apps Script（GAS）でCSVファイルとして出力される最終データと、バナー画像を含むzipファイルが保存されます。 |
| `07_SN_OUTPUT` | CSVファイル化の直前データ。ここの内容がGoogle Apps Script（GAS）でCSVファイルとして出力されます。 |
